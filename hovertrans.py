...
class HoverTrans(nn.Module):
    def __init__(self, img_size=224, patch_size=32, in_chans=1, num_classes=2, embed_dim=768, dim=48, depth=12,
        ...
        self.mapping_head = nn.Sequential(
                            nn.Conv2d(64, 768, 3, stride=1, padding=0),
                            nn.BatchNorm2d(768),
                            nn.ReLU(inplace=True),)
        for i in range(4):
            if i == 0:
    ...
    def forward(self, x):
        ...
        output1 = self.mapping_head(output)
        output_flat = self.avgpool(output1).flatten(1)
